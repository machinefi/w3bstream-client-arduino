...
#if defined(CONFIG_PSA_ITS_FLASH_C)
                                       ...
                                       struct psa_storage_info_t *p_info)
{
    ...
    n = its_flash->read(its_flash->start_address, (uid - 1) * IOTEX_HAL_FLASH_KEY_SLOT_SIZE, &header, sizeof( header ));
    if( n != sizeof( header ) )
                          ...
                          psa_storage_create_flags_t create_flags )
{
    ...
    n = its_flash->write(its_flash->start_address, (uid - 1) * IOTEX_HAL_FLASH_KEY_SLOT_SIZE, &header, sizeof( header ));
    if( n != sizeof( header ) )
        goto exit;
    if( data_length != 0 )
    {
        n = its_flash->write(its_flash->start_address, (uid - 1) * IOTEX_HAL_FLASH_KEY_SLOT_SIZE + sizeof( header ), p_data, data_length);
        if( n != data_length )
            goto exit;
    }
    status = PSA_SUCCESS;
